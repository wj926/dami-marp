...
스킬을 호출받으면 **본문 작성·부트스트랩 등 어떤 작업도 시작하기 전에** 사용자에게 테마를 묻는다.

물어보는 방식:
1. `~/.claude/skills/dami-marp/themes/*.css` 를 동적으로 스캔 (Glob 또는 `ls`)
2. 각 파일을 1, 2, 3 ... 번호로 사용자에게 제시
3. 마지막 옵션은 항상 **"새로 제작 (준비중)"**

예시 (현재 themes/ 에 dami-lab.css, aift-lab.css 두 개):
```
어떤 테마로 시작할까요?
  1. dami-lab     — 기본 (네이비, DAMI Lab 양식)
  2. aift-lab     — SKKU 진녹 (AIFT Lab)
  3. 새로 제작 (준비중)

번호로 답해주세요.
```

테마가 추가되면 번호도 자동으로 늘어남 (예: `themes/foo.css` 추가 시 4번이 foo, 5번이 새로 제작).

**3번(또는 마지막 번호 — 새로 제작) 선택 시**:
> "현재 새 테마 제작 기능은 준비중입니다. 1~N 중에서 선택해주세요."
> 안내 후 다시 같은 메뉴 제시 → 사용자가 기존 테마 중에서 고를 때까지 반복.

선택된 테마는 frontmatter `theme: <이름>` (확장자 .css 제외) 로 적용. 이 정보가 부트스트랩 → 본문 작성으로 이어짐.

> **새 테마 추가 향후 작업 (TODO)**: 현재 "새로 제작" 은 placeholder. 향후 사용자에게 색상·로고·기관명 몇 개 묻고 dami-lab.css 를 베이스로 프로그램적 변형 + `themes/` 에 신규 .css 저장하는 흐름을 추가할 예정. 그 전까지는 기존 테마만 선택 가능.
...
## 부속 기능 — 이미지 생성/삽입/사이즈 자동 조정 (`image-gen/`)

슬라이드를 만든 다음 → 그림으로 보여줬을 때 더 효과적인 부분을 ChatGPT에게 생성시키고 → 슬라이드에 박은 뒤 → 렌더링/캡쳐 결과를 보면서 사이즈를 자동 조정하는 5단계 루프.

상세: [image-gen/README.md](image-gen/README.md)

**7단계 워크플로우** (1단계는 필수, 2~7단계는 **선택**, **풀 자동화**):
1. **[필수]** 슬라이드 작성 (본 스킬) — 여기까지가 deliverable 핵심
2. **[선택]** 시각화 후보 추출 (아키텍처/플로우/매트릭스/타임라인 등)
3. **[선택]** `IMAGE_PROMPTS.md` 작성 — ChatGPT 프로필(`~/.cache/chatgpt-image-profile/`) 재활용
4. **[선택]** `image-gen/run.sh` 로 ChatGPT 이미지 일괄 생성
5. **[선택, 4 후속]** `python image-gen/auto_size.py path/to/slides.md` 으로 `![w:N]` 폭 자동 조정
6. **[선택, 4 후속]** Claude Code 자신이 슬라이드 PNG 를 `Read` 툴로 직접 봄 → 헐루시네이션·위치·구도·톤 체크리스트 평가 (외부 API 0개)
7. **[선택, 6 후속]** 평가 결과별 자동 액션: 프롬프트 재정제 → 단계 4 재실행, 또는 markdown 레이아웃 수정 → 재빌드 → 단계 6 재평가. 수렴 또는 max_iter 까지 반복

> **⚠️ 2~5단계는 시간·필요·에러 상황에 따라 자유롭게 스킵 가능**. 시간이 없거나, 텍스트만으로 충분히 명확하거나, ChatGPT 세션 만료/원격 호스트 다운/한글 헐루시네이션이 반복되면 망설이지 말고 그 시점 결과물로 빌드·제출. 이미지는 언제든 나중에 추가 후 재빌드 가능. 자세한 분기 가이드는 [image-gen/README.md#언제-건너뛰어도-되나](image-gen/README.md#언제-건너뛰어도-되나) 참고.

이미지가 많이 들어가는 발표자료(연구 발표, 사업계획서 등)에서만 선택적으로 사용. 본 스킬 단독으로도 충분하면 굳이 안 써도 됨.
